...
interface InvoiceUploadMissingImage { invoiceNumber: string; invoiceDate?: string; customerName: string; invoiceAmount: number; }
interface InvoiceUploadReconciliation { missingInvoiceImages: (InvoiceUploadMissingImage | string)[]; missingInvoiceImageKeys?: string[]; missingInvoices: string[]; }
...
@Component({
            ...
            <article *ngFor="let call of scheduledCalls" class="call-record-card" role="button" tabindex="0" (click)="openCallDetails(call)" (keydown)="onCallCardKeydown($event, call)">
              <div class="call-record-header">
                <div class="status-stack">
                  ...
                  <span *ngIf="isCallProspect(call)" class="status-badge prospect">Prospect</span>
                </div>
        ...
        <div class="card form-card">
          ...
          <form (ngSubmit)="saveNewCall()" class="sales-form">
            <div class="form-row">
              <label>Account Name *
                ...
                <datalist id="salesCustomerOptions">
                  <option *ngFor="let accountName of accountNameOptions" [value]="accountName"></option>
                </datalist>
              </label>
            ...
            <div class="form-grid-2">
              ...
              <label>Sales Rep *
                <select [(ngModel)]="newCall.repEmail" name="repEmail" required [disabled]="selectedCustomerHasAssignedRep || (!isSalesAdmin && salesReps.length > 0)">
                  <option *ngFor="let rep of salesReps" [value]="rep.repEmail || rep.email">{{rep.repName || rep.name || rep.repEmail || rep.email}} ({{rep.repEmail || rep.email}})</option>
                </select>
              </label>
            </div>
          ...
          <div class="history-filters-grid">
            ...
            <label>Account Search
              <div class="input-with-clear">
                ...
                <button *ngIf="historyAccountFilter" class="clear-btn" type="button" title="Clear account search" (click)="historyAccountFilter=''; onHistoryAccountSearchChange()">&times;</button>
              </div>
      ...
      <div class="card call-detail-modal" role="dialog" aria-modal="true" aria-labelledby="call-detail-title">
        ...
        <div class="call-detail-grid">
          <div class="call-detail-item">
            ...
            <span *ngIf="isCallProspect(selectedCallDetails)" class="status-badge prospect">Prospect</span>
          </div>
          <div class="call-detail-item">
        ...
        <div class="modal-actions">
          <button class="button secondary" type="button" (click)="closeCallDetails()">Close</button>
          <button *ngIf="isCallProspect(selectedCallDetails)" class="button secondary" type="button" (click)="convertProspectToCustomer()" [disabled]="convertingProspect">{{convertingProspect ? 'Adding...' : 'Add as Customer'}}</button>
            ...
            <div class="search-group">
              <label>Customer name
                <div class="input-with-clear">
                  ...
                  <button *ngIf="customerName" class="clear-btn" type="button" title="Clear customer name" (click)="customerName=''">&times;</button>
                </div>
              </label>
        ...
        <div class="invoice-results-col">
          ...
          <div *ngIf="invoices.length" class="card table">
            ...
            <tr>
              ...
              <th>Invoice</th>
              <th>Store</th>
              <th *ngIf="showCustomerColumns" class="sortable" (click)="toggleInvoiceSort('customerName')" tabindex="0" (keydown.enter)="toggleInvoiceSort('customerName')" (keydown.space)="$event.preventDefault(); toggleInvoiceSort('customerName')">Customer <span class="sort-indicator">{{invoiceSortKey === 'customerName' ? (invoiceSortDirection === 'asc' ? '↑' : '↓') : ''}}</span></th>
              <th *ngIf="showCustomerColumns" class="sortable" (click)="toggleInvoiceSort('customerNumber')" tabindex="0" (keydown.enter)="toggleInvoiceSort('customerNumber')" (keydown.space)="$event.preventDefault(); toggleInvoiceSort('customerNumber')">Customer no. <span class="sort-indicator">{{invoiceSortKey === 'customerNumber' ? (invoiceSortDirection === 'asc' ? '↑' : '↓') : ''}}</span></th>
              <th class="sortable" (click)="toggleInvoiceSort('invoiceDate')" tabindex="0" (keydown.enter)="toggleInvoiceSort('invoiceDate')" (keydown.space)="$event.preventDefault(); toggleInvoiceSort('invoiceDate')">Date <span class="sort-indicator">{{invoiceSortKey === 'invoiceDate' ? (invoiceSortDirection === 'asc' ? '↑' : '↓') : ''}}</span></th>
              <th class="sortable" (click)="toggleInvoiceSort('invoiceAmount')" tabindex="0" (keydown.enter)="toggleInvoiceSort('invoiceAmount')" (keydown.space)="$event.preventDefault(); toggleInvoiceSort('invoiceAmount')">Amount <span class="sort-indicator">{{invoiceSortKey === 'invoiceAmount' ? (invoiceSortDirection === 'asc' ? '↑' : '↓') : ''}}</span></th>
            </tr>
            <tr *ngFor="let invoice of invoices" class="invoice-row">
              ...
              <td *ngIf="showCustomerColumns" (click)="viewInvoice(invoice)">{{invoice.customerName}}</td>
              <td *ngIf="showCustomerColumns" (click)="viewInvoice(invoice)">{{invoice.customerNumber}}</td>
      ...
      <div class="card upload-panel">
        ...
        <div class="upload-reconciliation-grid">
          <div class="card upload-list-card">
            ...
            <div *ngIf="missingInvoiceImageDetails.length; else emptyInvoiceMissing" class="upload-missing-table-scroll">
              <table class="upload-missing-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr *ngFor="let invoice of missingInvoiceImageDetails">
                    <td><b>{{invoice.invoiceNumber}}</b></td>
                    <td>{{invoice.invoiceDate | date:'M/d/yyyy'}}</td>
  ...
  .call-success-toast {
    ...
    background: #f0fdf4;
    color: #166534;
    box-shadow: 0 12px 30px #14532d26;
    font-size: 0.9rem;
    font-weight: 700;
    animation: toast-in 0.2s ease-out;
  }
  .call-success-toast::before {
    content: '✓';
    display: grid;
    place-items: center;
    width: 1.35rem;
  ...
  .dark-theme .call-success-toast {
    border-color: #166534;
    background: #14532d;
    color: #dcfce7;
    box-shadow: 0 12px 30px #0005;
  }
  .dark-theme .call-success-toast::before {
    background: #4ade80;
    color: #14532d;
  }
  @keyframes toast-in {
  ...
  .sales-form .input-with-clear .clear-btn { width: auto; }
...
class AppComponent implements OnInit {
  ...
  get selectedCustomerHasAssignedRep(): boolean {
    return this.getAssignedRepEmails(this.findSalesCustomer(this.newCall.accountName)) .length > 0;
  }
  ...
  private runSearch(url: string, params: Record<string, string>) {
    ...
    this.http.get<Invoice[]>(url, { params }).subscribe({
      next: x => {
        this.invoices = x || [];
        this.sortInvoices();
      },
      error: e => this.error = e.status === 403 ? 'Update your password before using invoice lookup.' : 'Unable to search invoices.'
    });
  }

  toggleInvoiceSort(key: 'invoiceDate' | 'invoiceAmount' | 'customerName' | 'customerNumber') {
  ...
  private sortInvoices() {
    ...
    this.invoices = [...this.invoices].sort((a, b) => {
      const aValue = a[this.invoiceSortKey as keyof Invoice];
      const bValue = b[this.invoiceSortKey as keyof Invoice];
      ...
      if (this.invoiceSortKey === 'invoiceAmount') {
        return ((Number(a.invoiceAmount) || 0) - (Number(b.invoiceAmount) || 0)) * direction;
      }

      if (this.invoiceSortKey === 'customerName') {
  ...
  private findSalesCustomer(accountName: string | undefined): SalesCustomer | undefined {
    const normalizedName = (accountName || '').trim().toLowerCase();
    if (!normalizedName) return undefined;
    return this.salesCustomers.find(customer => this.getAccountName(customer).trim().toLowerCase() === normalizedName);
  }
  ...
  private applyAssignedRepForSelectedCustomer() {
    const assignedRepEmails = this.getAssignedRepEmails(this.findSalesCustomer(this.newCall.accountName));
    if (assignedRepEmails.length > 0) this.newCall.repEmail = assignedRepEmails[0];
  }

  isAssignedSalesCustomer(accountName: string): boolean {
    return this.getAssignedRepEmails(this.findSalesCustomer(accountName)).length > 0;
  }
  ...
  loadSalesData() {
    ...
    this.http.get<any[]>('/api/sales/customers').subscribe({
      next: custs => {
        ...
        this.applyAssignedRepForSelectedCustomer();
  ...
  onAccountNameChange() {
    ...
    const matched = this.findSalesCustomer(name);
    if (matched) {
      this.applyAssignedRepForSelectedCustomer();
  ...
  saveNewCall() {
    ...
    this.http.post<SalesCall>('/api/sales/calls', request).subscribe({
      next: () => {
  ...
  convertProspectToCustomer() {
    ...
    this.http.post(`/api/sales/calls/${callId}/convert-prospect`, {}).subscribe({
      next: () => {
  ...
  loadInvoiceStoreOptions() {
    this.http.get<number[]>('/api/invoices/stores').subscribe({
      ...
      error: () => {
        ...
        this.missingInvoiceImageDetails = [];
      }
    });
  }
  ...
  loadInvoiceReconciliation() {
    ...
    this.http.get<InvoiceUploadReconciliation>(`/api/invoices/upload-reconciliation?storeNumber=${this.selectedInvoiceUploadStore}`).subscribe({
      next: reconciliation => {
        ...
        const details = this.invoiceUploadReconciliation.missingInvoiceImages
          .filter((invoice): invoice is InvoiceUploadMissingImage => typeof invoice !== 'string');
        const keys = this.invoiceUploadReconciliation.missingInvoiceImageKeys
          || this.invoiceUploadReconciliation.missingInvoiceImages
            .filter((invoice): invoice is string => typeof invoice === 'string');
        this.loadMissingInvoiceImageDetails(details, keys);
      },
      error: () => {
        this.invoiceUploadReconciliation = { missingInvoiceImages: [], missingInvoices: [] };
        this.missingInvoiceImageDetails = [];
  ...
  private loadMissingInvoiceImageDetails(details: InvoiceUploadMissingImage[], invoiceKeys: string[]) {
    if (details.length || !invoiceKeys.length) {
      this.missingInvoiceImageDetails = details;
      return;
    }

    forkJoin(invoiceKeys.map(invoiceNumber =>
      this.http.get<Invoice[]>(`/api/invoices?invoiceNumber=${encodeURIComponent(invoiceNumber)}`)
    )).subscribe({
      next: results => {
        this.missingInvoiceImageDetails = results
          .flat()
          .map(invoice => ({
